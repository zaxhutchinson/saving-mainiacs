...
<html>
    ...
    <body>
        ...
        <?php
            ...
            echo DBManager::getInstance()->construct_dropdown("fRegionForm",
                    "fRegionList",
                    array("mammo_id"),
                    array("mammo_id"),
                    "SELECT distinct mammo_id FROM Region;");

            if(isset($_POST['formSubmit']) ){
                $lSelectedKey=filter_input(INPUT_POST, "fRegionList");
                $lKeyArray = explode(",",$lSelectedKey);
                //$lResult = MammoDB::getInstance()->get_mammogram_partitionfunction_data($lKeyArray[0]);
                $lResult = DBManager::getInstance()->query("SELECT * from Region WHERE mammo_id = " . $lKeyArray[0] . ";");
                $lColumnNames = array("mammo_id","region");
